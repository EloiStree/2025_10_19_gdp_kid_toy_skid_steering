class_name SkidCarMoveByDoubleWheelSize
extends Node
...
# --- CAR PARAMETERS ---
## If you use real physics value people cant play. To fast rotation.
@export var forward_amplified_value: float = 1.0
@export var backward_amplified_value: float = 0.3
...
@export var pivot_right_wheel_top_anchor: Node3D
...
var distance_per_second_amplified: float = 1.0
var distance_per_second_amplified_backward: float = 1.0


signal  on_left_wheel_degree_per_second_updated(new_value: float)
signal  on_right_wheel_degree_per_second_updated(new_value: float)
...
func compute_deducted_info() -> void:
	...
	wheel_height = pivot_right_front.global_position.distance_to(pivot_right_wheel_top_anchor.global_position)
	...
	distance_per_second_amplified = distance_per_second * forward_amplified_value
	distance_per_second_amplified_backward = distance_per_second * backward_amplified_value
	on_left_wheel_degree_per_second_updated.emit(wheel_rotation_angle_per_second)
	on_right_wheel_degree_per_second_updated.emit(wheel_rotation_angle_per_second)
...
func move_straight(delta: float, forward: bool = true) -> void:
	...
	if forward:
		move_dir = dir * distance_per_second_amplified * delta * (1 if forward else -1)
	else:
		move_dir = dir * distance_per_second_amplified_backward * delta * (1 if forward else -1)
